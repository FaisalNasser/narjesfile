...
class PageController extends Controller
{
    ...
    {
        $lan = session('locale') ?? json_decode(setting_by_key("languages"), true)[0];
        $pages = Page::where('language', $lan)->paginate(25);
    ...
    public function sort(Request $request)
    {
        try {
            ...
            //dd($request->sort);
            foreach ($pages as $pege) {
                foreach ($request->sort as $sort) {
                    //dd($sort['id'] . '  ' . $category->id);
                    if ($sort['id'] == $pege->id) {
                        $pege->update(['sort' => $sort['position']]);
                    }
                }
            }
